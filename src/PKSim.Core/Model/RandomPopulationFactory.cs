...
namespace PKSim.Core.Model
{
   ...
   {
      /// <summary>
      ///    Create a population using the specified <paramref name="populationSettings" />. The
      ///    <paramref name="cancellationToken" />
      ///    allows the caller to cancel the task
      /// </summary>
      Task<RandomPopulation> CreateFor(RandomPopulationSettings populationSettings, CancellationToken cancellationToken);
   }
   ...
   public class RandomPopulationFactory : IRandomPopulationFactory
   {
      ...
      private double tryCreateRandomValueFor(ParameterRange parameterRange, IParameter baseParameter, out bool success)
      {
         try
         {
            success = true;
            ...
            return baseParameter.RandomDeviateIn(_randomGenerator, parameterRange.MinValue, parameterRange.MaxValue);
         }
