...
public class HIDiseaseStateImplementation : IDiseaseStateImplementation
{
   ...
   private void updateBloodFlows(Individual individual, Action<IParameter, double> updateParameterFunc)
   {
   ...
   private void updateVolumes(Individual individual, Action<IParameter, double> updateParameterFunc)
   {
      var score = childPughScoreFor(individual);
      var organism = individual.Organism;
      var parameter = organism.Container(LIVER).Parameter(Constants.Parameters.VOLUME);
      updateParameterFunc(parameter, _hepaticVolumeScalingFactor[score]);
   }

   private Action<string, double> updateBloodFlowDef(Action<IParameter, double> updateParameterFunc, IContainer organism) => (organName, factor) =>
   {
      var parameter = organism.Container(organName).Parameter(SPECIFIC_BLOOD_FLOW_RATE);
      updateParameterFunc(parameter, parameter.Value * factor);
   };
