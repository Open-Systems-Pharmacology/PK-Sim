...
public class CalculationMethodCacheMapper : SnapshotMapperBase<ModelCalculationMethodCache, SnapshotCalculationMethodCache, CalculationMethodCacheSnapshotContext>
{
   ...
   public override Task<SnapshotCalculationMethodCache> MapToSnapshot(ModelCalculationMethodCache model) => mapCalculationMethodsToSnapshot(model);

   public virtual Task<SnapshotCalculationMethodCache> MapToSnapshot(ModelCalculationMethodCache model, string species) => mapCalculationMethodsToSnapshot(model, species);
   ...
   private Task<SnapshotCalculationMethodCache> mapCalculationMethodsToSnapshot(ModelCalculationMethodCache model, string species = null) =>
      SnapshotFrom(model, snapshot => { addCalculationMethodsToSnapshot(snapshot, model, species); });
