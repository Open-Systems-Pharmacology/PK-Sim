...
{
   /// <summary>
   ///    Brings projects saved before v13 in line with the new oral absorption and particle dissolution model. Every
   ///    building block that carries parameters the user can edit is aligned with the current database definition by
   ///    cloning the missing pieces from a template; nothing is recreated by hand. A simulation is not rebuilt from its
   ///    saved building blocks at load time, but its own copies are converted so that editing one of the new parameters
   ///    in the simulation and committing it back to the building block finds the parameter there.
   /// </summary>
      ...
      IVisitor<Simulation>
   {
      ...
      private void convertIndividual(Individual individual)
      {
         ...
         addLumenSegmentVolumeCalculationMethodTo(individual);
