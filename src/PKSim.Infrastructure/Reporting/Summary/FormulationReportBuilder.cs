...
namespace PKSim.Infrastructure.Reporting.Summary
{
   public class FormulationReportBuilder : ReportBuilder< PKSim.Core.Model.Formulation>
   {
      ...
      protected override void FillUpReport( PKSim.Core.Model.Formulation formulation, ReportPart reportPart)
      {
         ...
         {
            var particleDisperseSystem = formulation.Parameter(CoreConstants.Parameter.PARTICLE_DISPERSE_SYSTEM);
            var particleDistribution = formulation.Parameter(CoreConstants.Parameter.PARTICLE_SIZE_DISTRIBUTION);
            ...
            if (particleDisperseSystem.Value == CoreConstants.Parameter.MONODISPERSE)
            {
               ...
               tablePart.AddIs(formulation.Parameter(CoreConstants.Parameter.PARTICLE_RADIUS_MEAN), _infoRepository);
            }
            else
            {
               tablePart.AddIs(_infoRepository.DisplayNameFor(particleDisperseSystem), PKSimConstants.UI.Polydisperse);
               if (particleDistribution.Value == CoreConstants.Parameter.PARTICLE_SIZE_DISTRIBUTION_NORMAL)
               {
                  tablePart.AddIs(displayParticleDistribution, PKSimConstants.UI.Normal);
                  tablePart.AddIs(formulation.Parameter(CoreConstants.Parameter.PARTICLE_RADIUS_MEAN), _infoRepository);
                  tablePart.AddIs(formulation.Parameter(CoreConstants.Parameter.PARTICLE_RADIUS_STD_DEVIATION), _infoRepository);
               }
               else
               {
                  tablePart.AddIs(displayParticleDistribution, PKSimConstants.UI.LogNormal);
                  tablePart.AddIs(formulation.Parameter(CoreConstants.Parameter.PARTICLE_LOG_DISTRIBUTION_MEAN), _infoRepository);
                  tablePart.AddIs(formulation.Parameter(CoreConstants.Parameter.PARTICLE_LOG_VARIATION_COEFF), _infoRepository);
               }
            }
         }
