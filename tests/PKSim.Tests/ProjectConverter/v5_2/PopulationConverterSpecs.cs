...
namespace PKSim.ProjectConverter.v5_2
{
   ...
   public class When_converting_a_population_from_5_1_to_5_2 : concern_for_PopulationConverter
   {
      ...
      public void should_have_removed_the_ontogeny_factor_from_all_container_in_the_individual()
      {
         ...
         {
            molecule.Parameter(CoreConstants.Parameter.ONTOGENY_FACTOR).ShouldNotBeNull();
            if(molecule.Name!=CoreConstants.Molecule.UndefinedLiver)
               molecule.Parameter(CoreConstants.Parameter.ONTOGENY_FACTOR_GI).ShouldNotBeNull();

            foreach (var container in molecule.AllExpressionsContainers())
            {
               container.Parameter(CoreConstants.Parameter.ONTOGENY_FACTOR).ShouldBeNull();
            }
         }
      }
      ...
      public void should_have_adjusted_the_path_of_the_individual_parameters_in_the_population()
      {
         foreach (var molecule in _individual.AllMolecules())
         {
            ...
            _population.IndividualPropertiesCache.Has(string.Format("{0}|{1}", molecule.Name, CoreConstants.Parameter.ONTOGENY_FACTOR)).ShouldBeTrue();
            _population.IndividualPropertiesCache.Has(string.Format("{0}|{1}", molecule.Name, CoreConstants.Parameter.ONTOGENY_FACTOR_GI)).ShouldBeTrue();
         }
      }
      ...
      public void should_have_removed_the_old_ontogeny_factor_path()
      {
         foreach (var molecule in _individual.AllMolecules())
         {
            ...
            _population.IndividualPropertiesCache.Has(string.Format("{0}|Liver|{1}", molecule.Name, CoreConstants.Parameter.ONTOGENY_FACTOR)).ShouldBeFalse();
            _population.IndividualPropertiesCache.Has(string.Format("{0}|Duodenum|{1}", molecule.Name, CoreConstants.Parameter.ONTOGENY_FACTOR)).ShouldBeFalse();
            _population.IndividualPropertiesCache.Has(string.Format("{0}|Lumen-Duodenum|{1}", molecule.Name, CoreConstants.Parameter.ONTOGENY_FACTOR)).ShouldBeFalse();
         }
      }
   }
}