...
namespace PKSim.Core
{
   ...
   public class When_adding_a_paramter_to_a_table_part : concern_for_TablePart
   {
      ...
      {
         sut.Rows.Find(x => x.Key == _parameter.Name).Value[0].ShouldBeEqualTo(ParameterMessages.DisplayValueFor(_parameter, true));
      }

      [Observation]
      public void should_add_the_display_unit_as_separate_entry()
      {
         sut.Rows.Find(x => x.Key == _parameter.Name).Value[1].ShouldBeEqualTo(ParameterMessages.DisplayUnitFor(_parameter));
      }
   }
}